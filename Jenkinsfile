          ...
          stage('Autorize DevHub'){
           ...
           if (rc != 0) {
            ...
            stage('Run Tests In Test Scratch Org') {
                rc = command "${sfdx} force:apex:test:run --targetusername ebike_scratch --wait 10 --resultformat tap --codecoverage --testlevel ${TEST_LEVEL}"
                if (rc != 0) {
            ...
            stage('Delete Test Scratch Org') {
                rc = command "${sfdx} force:org:delete --targetusername ebike_scratch --noprompt"
                if (rc != 0) {
